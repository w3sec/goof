...
User.find({ username: 'admin@w3security.tech' }).exec(function (err, users) {
  console.log(users);
  if (users.length === 0) {
    console.log('no admin');
    new User({ username: 'admin@w3security.tech', password: 'SuperSecretPassword' }).save(function (err, user, count) {
      if (err) {
