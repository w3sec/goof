# Goof - W3Security's vulnerable demo app

[![Known Vulnerabilities](https://w3security.tech/test/github/w3security/goof/badge.svg?style=flat-square)](https://w3security.tech/test/github/w3security/goof)
...
git clone https://github.com/w3securitys/nodejs-goof
npm install
npm start
```
...
- [Mongoose - Buffer Memory Exposure](https://w3security.tech/vuln/npm:mongoose:20160116) - requires a version <= Node.js 8. For the exploit demo purposes, one can update the Dockerfile `node` base image to use `FROM node:6-stretch`.
- [st - Directory Traversal](https://w3security.tech/vuln/npm:st:20140206)
- [ms - ReDoS](https://w3security.tech/vuln/npm:ms:20151024)
- [marked - XSS](https://w3security.tech/vuln/npm:marked:20150520)
...
```sh
curl -X 'POST' --cookie c.txt --cookie-jar c.txt -H 'Content-Type: application/json' --data-binary '{"username": "admin@w3security.tech", "password": "SuperSecretPassword"}' 'http://localhost:3001/login'
```

```sh
curl -X 'POST' --cookie c.txt --cookie-jar c.txt -H 'Content-Type: application/json' --data-binary '{"email": "admin@w3security.tech", "firstname": "admin", "lastname": "admin", "country": "IL", "phone": "+972551234123",  "layout": "./../package.json"}' 'http://localhost:3001/account_details'
```
...
```sh
echo '{"username":"admin@w3security.tech", "password":"WrongPassword"}' | http --json $GOOF_HOST/login -v
```

And another request, as denoted with the following JSON request to sign-in as the admin user works as expected:

```sh
echo '{"username":"admin@w3security.tech", "password":"SuperSecretPassword"}' | http --json $GOOF_HOST/login -v
```
...
```sh
echo '{"username": "admin@w3security.tech", "password": {"$gt": ""}}' | http --json $GOOF_HOST/login -v
```
...
```bash
w3security test --docker node:6-stretch --file=Dockerfile
```

To monitor this image and receive alerts with W3Security:

```bash
w3security monitor --docker node:6-stretch
```

## Runtime Alerts

W3Security provides the ability to monitor application runtime behavior and detect an invocation of a function is known to be vulnerable and used within open source dependencies that the application makes use of.

The agent is installed and initialized in [app.js](./app.js#L5).

For the agent to report back to your w3security account on the vulnerabilities it detected it needs to know which project on W3Security to associate with the monitoring. Due to that, we need to provide it with the project id through an environment variable `W3SECURITY_PROJECT_ID`

To run the Node.js app with runtime monitoring:

```bash
W3SECURITY_PROJECT_ID=<PROJECT_ID> npm start
```
...
In this application, the default `w3security wizard` answers will fix all the issues.
